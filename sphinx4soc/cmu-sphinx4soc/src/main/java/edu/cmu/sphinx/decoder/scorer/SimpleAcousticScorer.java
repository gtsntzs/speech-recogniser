...
{
    static final Logger loger = Logger.getLogger( SimpleAcousticScorer.class );
    ...
    public Data calculateScoresSoA ( ActiveList activeList, Data data )
    {
        try {
            if ( data instanceof DataStartSignal||data instanceof DataEndSignal ) {
                return data;        // Create an empty object
            }

            List<? extends Scoreable> scoreableList = activeList.getTokens();

            if ( scoreableList.isEmpty() ) {
                return null;        // Create an empty object
            }
...
//            // apply optional score normalization
    ...
    {
        loger.info( "Allocating SimpleAcousticScorer" );
        loger.info( "Nothing to be done." );
        loger.info( "SimpleAcousticScorer Allocated" );
    }
