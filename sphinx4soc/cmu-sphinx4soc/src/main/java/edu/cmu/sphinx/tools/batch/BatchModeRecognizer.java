...
{
    static final Logger logger = Logger.getLogger( BatchModeRecognizer.class );
    /**
...
//        logger = ps.getLogger();
    ...
    public void setBatchFile ( String batchFile ) throws IOException
    {
        if ( usePooledBatchManager ) {
            ...
            batchManager = new SimpleBatchManager( batchFile, skip, whichBatch, totalBatches );
        }
    }
    ...
    public void decode ( String batchFile ) throws IOException
    {
        ...
        try {
            ...
            logger.error( "I/O error during decoding: "+io.getMessage() );
            throw io;
        }
...
//                    System.err.println("tried to configure the CM with " + args );
    ...
    public Result recognize () throws IOException
    {
        Result result = null;
        BatchItem batchItem;
        if ( count<utteranceId&&( batchItem = batchManager.getNextItem() )!=null ) {
            setInputStream( batchItem.getFilename() );
