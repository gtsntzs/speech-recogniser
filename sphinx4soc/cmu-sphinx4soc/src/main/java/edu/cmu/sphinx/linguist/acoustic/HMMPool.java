...
{
    static final Logger logger = Logger.getLogger( HMMPool.class );
    ...
    private Unit synthesizeUnit ( int id )
    {
        ...
        Unit unit = unitManager.getUnit( centralUnit.getName(), centralUnit.isFiller(), context );

        logger.debug( "Missing "+getUnitNameFromID( id )+" returning "+unit );
        return unit;
    }
    ...
    public HMM getHMM ( Unit base, Unit lc, Unit rc, HMMPosition pos )
    {
        ...
        if ( !isValidID( bid ) ) {
            logger.error( "Bad HMM Unit: "+base.getName() );
            return null;
        }
        if ( !isValidID( lid ) ) {
            logger.error( "Bad HMM Unit: "+lc.getName() );
            return null;
        }
        if ( !isValidID( rid ) ) {
            logger.error( "Bad HMM Unit: "+rc.getName() );
            return null;
        }
        id = buildID( bid, lid, rid );
        if ( id<0 ) {
            logger.error( "Unable to build HMM Unit ID for "+base.getName()+" lc="+lc.getName()+" rc="+rc.getName() );
            return null;
        }
        HMM hmm = getHMM( id, pos );
        if ( hmm==null ) {
            logger.error( "Missing HMM Unit for "+base.getName()+" lc="+lc.getName()+" rc="+rc.getName() );
        }
    ...
    public void dumpInfo ()
    {
        ...
        for ( int i = 0; i<unitTable.length; i++ ) {
            logger.debug( String.valueOf( i )+' '+unitTable[i] );
        }
    }
    /**
