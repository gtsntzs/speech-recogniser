...
public class DataUtil
{
    ...
    private static String doubleArrayToString ( double[] data, int format )
    {
        ...
        for ( double val : data ) {
            if ( format==DECIMAL ) {
                dump.append( ' ' ).append( formatDouble( val, decimalIntegerDigits,
                        decimalFractionDigits ) );
            } else if ( format==HEXADECIMAL ) {
                long binary = Double.doubleToRawLongBits( val );
                dump.append( " 0x" ).append( Long.toHexString( binary ) );
            } else if ( format==SCIENTIFIC ) {
                dump.append( ' ' ).append( Utilities.doubleToScientificString( val, doubleScientificFractionDigits ) );
            }
        }
        return dump.toString();
    }
    ...
    private static String floatArrayToString ( float[] data, int format )
    {
        ...
        for ( float val : data ) {
            if ( format==DECIMAL ) {
                dump.append( ' ' ).append( formatDouble( (double)val,
                ...
                dump.append( ' ' ).append( Utilities.doubleToScientificString( (double)val, floatScientificFractionDigits ) );
            }
        }
        return dump.toString();
    }
