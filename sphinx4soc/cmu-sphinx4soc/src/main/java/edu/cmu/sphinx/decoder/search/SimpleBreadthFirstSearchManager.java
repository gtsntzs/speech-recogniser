...
{
        static final Logger logger = Logger.getLogger( SimpleBreadthFirstSearchManager.class );
...
//        logger = ps.getLogger();
        ...
        public void startRecognitionSoA() // add the number of frames to recognise
        {
        ...
        protected ActiveList undoLastGrowStep()
        {
                ...
                for ( Token token : activeList ) {
                        ...
                        while ( curToken.getPredecessor() != null && ((curToken.isFinal() && curToken.getPredecessor() != null && !curToken.getPredecessor().isFinal())
        ...
        public Result createResult()
        {
                ...
                Result result = null;
//        logger.info( activeList.getBestToken() );
                // generate a new temporary result if the current token is based on a final search state
                // remark: the first check for not null is necessary in cases that the search space does not contain scoreable tokens.
                if ( activeList.getBestToken() != null ) {
                        // to make the current result as correct as possible we undo the last search graph expansion here
                        ...
                        // now create the result using the fixed active-list
                        result = new Result( fixedList, resultList, currentFrameNumber, true, logMath );
        ...
        {
                boolean hasMoreFrames = false; // flag specifing if there are more frames
                ...
                logger.debug( currentFrameNumber + " " + activeList.size() + " " + curTokensScored.value + " " + (int) tokensPerSecond.value );
        ...
        public boolean scoreTokensSoA( Data token )
        {
                boolean hasMoreFrames = false; // flag specifing if there are more frames
        ...
        protected void growBranches()
        {
                ...
                totalHmms += hmms;
                logger.debug( "Frame: " + currentFrameNumber + " Hmms: " + hmms + "  total " + totalHmms );
        }
        ...
        protected Token getBestToken( SearchState state )
        {
                ...
                logger.debug( "BT " + best + " for state " + state );
                return best;
        }
...
//    public Exchange getActiveListSoA ( Exchange exchange )
//    {
//        System.out.println( "xaxaxaxaxxaaxa" );
...
//    }
        ...
        public void allocateSoA()
        {
                logger.info( "Allocating SimpleBreadthFirstSearchManager" );
...
//        }
                ...
                logger.info( "SimpleBreadthFirstSearchManager Allocated" );
        }
