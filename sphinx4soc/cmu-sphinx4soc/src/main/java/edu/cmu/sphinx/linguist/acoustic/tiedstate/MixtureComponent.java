...
public class MixtureComponent implements Cloneable, Serializable {
     ...
     * <p/>
     * Note: The support of
     * <code>DoubleData</code>-features would require an array conversion to float[]. Because
     * getScore might be invoked with very high frequency, features are restricted to be
     * <code>FloatData</code>s.
     *
     ...
     * the computation of a multi-dimensional Gaussian.
     * <p/>
     * <p><b>Normal(x) = exp{-0.5 * (x-m)' * inv(Var) * (x-m)} / {sqrt((2 * PI) ^ N) *
     * det(Var))}</b></p>
     * <p/>
     ...
     * the diagonal elements are non-zero - is simply the product of the diagonal elements.
     * <p/>
    ...
    /**
     * Applies transformations to means and variances. The transformed mean vector is given by:
     *
     * <p><b>M = A * m + B</b></p>
     *
     * where <b>M</b> and <b>m</b> are the mean vector after and before transformation,
     * respectively, and <b>A</b> and <b>B</b> are the transformation matrix and vector,
     * respectively.
     *
     * if A or B are
     * <code>null</code> the according substeps are skipped
     */
    public void transformStats() {
        ...
        /**
         * We do analogously with the variance. In this case, we also invert the variance, and work
         * with precision instead of variance.
         */
