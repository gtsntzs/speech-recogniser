...
{
    private static final Logger log = Logger.getLogger( WordPruningBreadthFirstSearchManager.class );
    ...
    public Result recognize ( int nFrames )
    {
        ...
        if ( !streamEnd ) {
            result = new Result( loserManager, activeList, resultList, currentFrameNumber, done, logMath );
        }
...
//        log.info( "Stoping" );
    ...
    protected void growBranches ()
    {
        ...
        logger.debug( "Frame: "+currentFrameNumber+" thresh : "+relativeBeamThreshold+" bs "+activeList.getBestScore()+" tok "
    ...
    protected void growNonEmittingBranches ()
    {
...
//            log.info( "Average Tokens/State: "+( tokenSum/tokenCount ) );
        }
    }
        ...
        for ( SearchStateArc arc : arcs ) {
            ...
            if ( firstToken||bestToken.getScore()<logEntryScore ) {
                Token newBestToken = new Token( predecessor, nextState,
                        logEntryScore, arc.getInsertionProbability(), arc.getLanguageProbability(), currentFrameNumber );
                tokensCreated.value++;
