...
public abstract class AbstractDecoder implements ResultProducer, Configurable
{
    ...
    public AbstractDecoder ( SearchManager searchManager, boolean fireNonFinalResults, boolean autoAllocate, List<ResultListener> resultListeners )
    {
        ...
        init( name, Logger.getLogger( name ), searchManager, fireNonFinalResults, autoAllocate, resultListeners );
    }
    ...
    {
        init( ps.getInstanceName(),
                ps.getLogger(),
                (SearchManager)ps.getComponent( PROP_SEARCH_MANAGER ),
                ps.getBoolean( FIRE_NON_FINAL_RESULTS ),
                ps.getBoolean( AUTO_ALLOCATE ),
                ps.getComponentList( PROP_RESULT_LISTENERS, ResultListener.class ) );
    }
