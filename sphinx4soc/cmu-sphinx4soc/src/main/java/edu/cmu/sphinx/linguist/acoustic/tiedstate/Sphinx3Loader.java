 ...
 * <p>
 * The acoustic model is stored as a directory specified by a URL.
 * The dictionary and language model files are not required to be in the
 * package. You can specify their locations separately. 
 * </p>
 * <p>Configuration file should set mandatory property of component:
 * <ul>
 ...
 * "transition_matrices", "mixture_weights" for binary versions, or with suffix
 * ".ascii" for the ASCII versions.
 * </p>
 * <p>
 * If model has some layout that is different default generated by SphinxTrain, you may 
 * specify additional properties like <b>dataLocation</b> to set the path to the binary 
 * files, <b>mdef</b> to set the path to the model definition file.
 * </p>
 * <p>
...
//    protected Logger logger ;
    ...
    public Sphinx3Loader ( URL location, String model, String dataLocation, LogMath logMath, 
    		UnitManager unitManager, boolean isBinary, boolean sparseForm, int vectorLength, 
    		float distFloor, float mixtureWeightFloor, float varianceFloor, boolean useCDUnits )
    {

        init( location, model, dataLocation, logMath, unitManager, isBinary, sparseForm, 
        		vectorLength, distFloor, mixtureWeightFloor, varianceFloor, useCDUnits );
    }

    public Sphinx3Loader ( String location, String model, String dataLocation, LogMath logMath, 
    		UnitManager unitManager, boolean isBinary, boolean sparseForm, int vectorLength, 
    		float distFloor, float mixtureWeightFloor, float varianceFloor, boolean useCDUnits ) 
    				throws MalformedURLException, ClassNotFoundException
    {
    	
        init( LoadResourceUtils.load( location ), model, dataLocation, logMath, 
        		unitManager, isBinary, sparseForm, vectorLength, distFloor, mixtureWeightFloor, 
        		varianceFloor, useCDUnits );
    }

    protected void init ( URL location, String model, String dataLocation, LogMath logMath, 
    		UnitManager unitManager, boolean isBinary, boolean sparseForm, int vectorLength, 
    		float distFloor, float mixtureWeightFloor, float varianceFloor, boolean useCDUnits )
    {
    ...
    {
        if ( !loaded ) {
            logger.info( "Loading Sphinx3Loader" );
            ...
            logger.info( "Sphinx3Loader Loaded" );
        }
    }
     ...
     * 
     * @param modelDef
     *            the name of the acoustic modelDef; if null we just load from the
     *            default location
     * @throws java.io.IOException
     */
    private void loadModelFiles ( String modelDef ) throws IOException, URISyntaxException
    {
        logger.info( "Loading Sphinx3 acoustic model: "+modelDef );
        logger.info( "    modelName: "+this.model );
        logger.info( "    dataLocation   : "+dataLocation );

        if ( binary ) {
            meansPool = loadDensityFileBinary( dataLocation+"means", -Float.MAX_VALUE );
            variancePool = loadDensityFileBinary( dataLocation+"variances", varianceFloor );
            mixtureWeightsPool = loadMixtureWeightsBinary( dataLocation+"mixture_weights", mixtureWeightFloor );
            matrixPool = loadTransitionMatricesBinary( dataLocation+"transition_matrices" );
            transformMatrix = loadTransformMatrix( dataLocation+"feature_transform" );
        } else {
            meansPool = loadDensityFileAscii( dataLocation+"means.ascii", -Float.MAX_VALUE );
            variancePool = loadDensityFileAscii( dataLocation+"variances.ascii", varianceFloor );
            mixtureWeightsPool = loadMixtureWeightsAscii( dataLocation+"mixture_weights.ascii", mixtureWeightFloor );
            matrixPool = loadTransitionMatricesAscii( dataLocation+"transition_matrices.ascii" );
        }
    ...
    private Pool<Senone> createSenonePool ( float distFloor, float varianceFloor )
    {
        ...
        logger.info( "Num Gaussians Per Senone = "+numGaussiansPerSenone );
        logger.info( "Num Senones = "+numSenones );
        logger.info( "Num Mixture Weights = "+numMixtureWeights );
        logger.info( "Num Means = "+numMeans );
        logger.info( "Num Variances = "+numVariances );
        ...
        for ( int i = 0; i<numSenones; i++ ) {
            ...
            for ( int j = 0; j<numGaussiansPerSenone; j++ ) {
                mixtureComponents[j] = new MixtureComponent( logMath, meansPool.get( whichGaussian ),
    ...
    {
        logger.info( "Loading density file from: "+path );
            ...
            throws IOException, URISyntaxException
    {
        ...
        logger.info( "Number of states "+numStates );
        logger.info( "Number of streams "+numStreams );
        logger.info( "Number of gaussians per state "+numGaussiansPerState );
        logger.info( "Vector length "+vectorLength.length );
        logger.info( "Raw length "+rawLength );
                                 ...
                                 String path ) throws IOException
    {
        ...
        logger.info( "Loading HMM file from: "+path );
...
//                }
            ...
            throws IOException, URISyntaxException
    {
        ...
        int numStates = est.getInt( "numStates" );
        logger.info( "with "+numMatrices+" and "+numStates+" states, in "+( sparseForm ? "sparse" : "dense" )+" form" );
...
//                    }
    ...
    {
        logger.info( "Loading transform matrix from: "+path );
