 ...
 * signal over its entrie duration, we analyze one <b>window</b> of audio data.
 ...
 * <p/>
 * By default, the number of FFT points is the closest power pf 2 that is equal
 ...
 * windowed signal.
 * <p/>
 * For example, Figure 1 below shows the audio signal of the utterance
 * "one three nine oh", and Figure 2 shows its spectrogram, produced by putting
...
public class DiscreteFourierTransform extends BaseDataProcessor {
	...
	private DoubleData process(DoubleData input)
		 ...
		 */
		DoubleData output = new DoubleData(outputSpectrum,
				input.getSampleRate(), input.getCollectTime(),
	 ...
	 *            whether it's direct (false) or inverse (true) FFT
	 */
	private void recurseFft(Complex[] input, double[] output,
