...
/**
 * <p/>
 * A Microphone captures audio data from the system's underlying audio input systems. Converts these
 * audio data into Data objects. When the method <code>startRecording()</code> is called, a new
 * thread will be created and used to capture audio, and will stop when <code>stopRecording()</code>
 * is called. Calling <code>getData()</code> returns the captured audio data as Data objects.
 * </p>
 * <p/>
 * This Microphone will attempt to obtain an audio device with the format specified in the
 * configuration. If such a device with that format cannot be obtained, it will try to obtain a
 * device with an audio format that has a higher sample rate than the configured sample rate, while
 * the other parameters of the format (i.e., sample size, endianness, sign, and channel) remain the
 * same. If, again, no such device can be obtained, it flags an error, and a call
 * <code>startRecording</code> returns false.
 * </p>
 */
public class Microphone extends BaseDataProcessor implements DataProvider {
    ...
    /**
     * The property that specifies whether or not the microphone will release the audio between
     * utterances. On certain systems (Linux for one), closing and reopening the audio does not work
    ...
    /**
     * The property that specifies the number of milliseconds of audio data to read each time from
    ...
    /**
     * The property that specifies whether to keep the audio data of an utterance around until the
     * next utterance is recorded.
     */
    @S4Boolean(defaultValue = false)
    public final static String PROP_KEEP_LAST_AUDIO = "keepLastAudio";
    /**
     * The property that specifies how to convert stereo audio to mono. Currently, the possible
     * values are "average", which averages the samples from at each channel, or "selectChannel",
     * which chooses audio only from that channel. If you choose "selectChannel", you should also
     * specify which channel to use with the "selectChannel" property.
     */
    ...
    /**
     * The property that specifies the mixer to use. The value can be "default," (which means let
     * the AudioSystem decide), "last," (which means select the last Mixer supported by the
     * AudioSystem), which appears to be what is often used for USB headsets, or an integer value
     * which represents the index of the Mixer.Info that is returned by AudioSystem.getMixerInfo().
     * To get the list of Mixer.Info objects, run the AudioTool application with a command line
    ...
    /**
     * The property that specifies the size of the buffer used to store audio samples recorded from
     * the microphone. Default value correspond to 200ms. Smaller value decrease microphone latency
     * with danger of dropping out the frames if decoding thread will be slow enough to process the
     * result.
     */
     ...
     *            certain systems (Linux for one), closing and reopening the audio does not work too
     ...
     * @param keepLastAudio
     *            whether to keep the audio data of an utterance around until the next utterance is
     *            recorded.
     * @param stereoToMono
     *            how to convert stereo audio to mono. Currently, the possible values are "average",
     *            which averages the samples from at each channel, or "selectChannel", which chooses
     *            audio only from that channel. If you choose "selectChannel", you should also
     *            specify which channel to use with the "selectChannel" property.
     * @param selectedChannel
     *            the channel to use if the audio is stereo
     * @param selectedMixerIndex
     *            the mixer to use. The value can be "default," (which means let the AudioSystem
     *            decide), "last," (which means select the last Mixer supported by the AudioSystem),
     *            which appears to be what is often used for USB headsets, or an integer value which
     *            represents the index of the Mixer.Info that is returned by
     *            AudioSystem.getMixerInfo(). To get the list of Mixer.Info objects, run the
     *            AudioTool application with a command line argument of "-dumpMixers".
     */
    public Microphone(int sampleRate, int bitsPerSample, int channels, boolean bigEndian, boolean signed, boolean closeBetweenUtterances,
            int msecPerRead, boolean keepLastAudio, String stereoToMono, int selectedChannel, String selectedMixerIndex, int audioBufferSize) {
        initLogger();
    ...
    public void create() {
        this.desiredFormat = new AudioFormat((float) sampleRate, this.bitsPerSample, channels, signed, bigEndian);
    ...
    public void initialize() {
        ...
        if (!AudioSystem.isLineSupported(info)) {
            loger.info(desiredFormat + " not supported");
            AudioFormat nativeFormat = DataUtil.getNativeAudioFormat(desiredFormat, getSelectedMixer());
            if (nativeFormat == null) {
                loger.error("couldn't find suitable target audio format");
            } else {
                ...
                if (doConversion) {
                    loger.info("Converting from " + finalFormat.getSampleRate() + "Hz to " + desiredFormat.getSampleRate() + "Hz");
                } else {
                    loger.info("Using native format: Cannot convert from " + finalFormat.getSampleRate() + "Hz to " + desiredFormat.getSampleRate()
                            + "Hz");
                }
            }
        } else {
            loger.info("Desired format: " + desiredFormat + " supported.");
    ...
    private TargetDataLine getAudioLine() {
        ...
        try {
             ...
             */
            loger.info("Final format: " + finalFormat);
            ...
            audioLine.addLineListener(new LineListener() {
                ...
                public void update(LineEvent event) {
                    loger.info("line listener " + event);
                }
            });
        } catch (LineUnavailableException e) {
            loger.error("microphone unavailable " + e.getMessage());
        }
    ...
    private boolean open() {
        ...
        if (audioLineTmp != null) {
            if (!audioLineTmp.isOpen()) {
                loger.info("open");
                try {
                    audioLineTmp.open(finalFormat, audioBufferSize);
                } catch (LineUnavailableException e) {
                    loger.error("Can't open microphone " + e.getMessage());
                    return false;
                }
                ...
                frameSizeInBytes = (audioStream.getFormat().getSampleSizeInBits() / 8) * (int) (sec * audioStream.getFormat().getSampleRate());

                loger.info("Frame size: " + frameSizeInBytes + " bytes");
            }
            return true;
        } else {
            loger.error("Can't find microphone");
            return false;
        }
    }

    /**
     * Returns the format of the audio recorded by this Microphone. Note that this might be
     * different from the configured format.
     *
    ...
    /**
     * Starts recording audio. This method will return only when a START event is received, meaning
     * that this Microphone has started capturing audio.
     *
    ...
    public synchronized boolean startRecording() {
        ...
        if (audioLine.isRunning()) {
            loger.warn("Whoops: audio line is running");
        }
    ...
    /**
     * Stops recording audio. This method does not return until recording has been stopped and all
     * data has been read from the audio line.
     */
    ...
    class RecordingThread extends Thread {
        ...
        /**
         * Stops the thread. This method does not return until recording has actually stopped, and
         * all the data has been read from the audio line.
         */
        public void stopRecording() {
            ...
            // flush can not be called here because the audio-line might has been set to null
            // already by the mic-thread
            // audioLine.flush();
        }

        /**
         * Implements the run() method of the Thread class. Records audio, and cache them in the
         * audio buffer.
         */

        public void run() {
            totalSamplesRead = 0;
            loger.info("started recording");
            ...
            loger.info("DataStartSignal added");
            try {
                audioLine.start();
                while (!done) {
                ...
                loger.warn("IO Exception " + ioe.getMessage());
            }
            ...
            loger.info("DataEndSignal ended");
            loger.info("stopped recording");
        ...
        private synchronized void waitForStart() {
            ...
            try {
                ...
                loger.warn("wait was interrupted");
            }
        }
        ...
        private Data readData(Utterance utterance) throws IOException {
            ...
            loger.debug("Read " + numBytesRead + " bytes from audio stream.");
    ...
    /**
     * Reads and returns the next Data object from this Microphone, return null if there is no more
     * audio data. All audio data captured in-between <code>startRecording()</code> and
     * <code>stopRecording()</code> is cached in an Utterance object. Calling this method basically
     * returns the next chunk of audio data cached in this Utterance.
     *
    ...
    /**
     * Returns true if there is more data in the Microphone. This happens either if the a
     * DataEndSignal data was not taken from the buffer, or if the buffer in the Microphone is not
     * yet empty.
     *
