...
{
    static final Logger logger = Logger.getLogger( FlatLinguist.class );
    /**
...
//    
    public void allocateSoA () throws IOException
    {
        logger.info( "Allocating FlatLinguist" );
        ...
        logger.info( "FlatLinguist Allocated" );
    ...
    public void startRecognition ()
    {
        ...
        if ( grammarHasChanged() ) {
            logger.info( "Grammar has changed, compiling grammar." );
            stateSet = compileGrammar();
            totalStates.value = stateSet.size();
        }
    }
...
//                    int maxSize = getRightContextSize();
        ...
        {
            prevState.connect( getArc( nextState, logLanguageProbablity, logInsertionProbablity ) );
