...
public class ConcatFileDataSource extends StreamDataSource implements ReferenceSource
{
    ...
    public void initialize ()
    {
        ...
        try {
            ...
            setInputStream( new SequenceInputStream( new InputStreamEnumeration( batchFile, startFile, totalFiles ) ), batchFile );
    ...
    class InputStreamEnumeration implements Enumeration<InputStream>
    {
        ...
        {
            try {
                if ( 0<=totalFiles&&totalFiles<=referenceList.size() ) {
                    return null;
                }
        ...
        private void writeTranscript ( String fileName, String reference )
        {
            try {
                ...
                transcript.write( context+" 1 "+fileName+' '+start+' '+end+"  "+reference+'\n' );
                transcript.flush();
            } catch ( IOException ioe ) {
            }
        }
        ...
        private void writeSilenceToTranscript ()
        {
            try {
                ...
                float end = getSeconds( totalBytes );
                transcript.write( context+" 1 "+GAP_LABEL+' '+start+' '+end+" \n" );
                transcript.flush();
            } catch ( IOException ioe ) {
            }
        }
