 ...
 * followed by the pronunciation (by way of the list of phones) of the word.
 * Each word can have more than one pronunciations. For example, a digits
...
public class FullDictionary implements Dictionary {
	...
	public FullDictionary(URL wordDictionaryFile, URL fillerDictionaryFile,
	...
	public FullDictionary(String wordDictionaryFile, String fillerDictionaryFile,
			List<URL> addendaUrlList, boolean addSilEndingPronunciation,
			String wordReplacement, boolean allowMissingWords,
			boolean createMissingWords, UnitManager unitManager) throws MalformedURLException {

		this.wordDictionaryFile = LoadResourceUtils.load(wordDictionaryFile);
		this.fillerDictionaryFile = LoadResourceUtils.load(fillerDictionaryFile);
	...
	public void allocate() throws IOException {
		...
		if (!allocated) {
			...
			fillerDictionary = loadDictionary(
					fillerDictionaryFile.openStream(), true);
			loadTimer.stop();
			logger.debug(dumpToString());
			allocated = true;
		}
	}
	 ...
	 */
	protected Map<String, Word> loadDictionary(InputStream inputStream,
		...
		while ((word = est.getString()) != null) {
			...
			if (!isFillerDict && addSilEndingPronunciation) {
				...
				Unit[] unitsArray2 = units.toArray(new Unit[units.size()]);
	 ...
	 */
	protected HashMap<String, Word> createWords(
			Map<String, List<Pronunciation>> pronunciationList,
			boolean isFillerDict) {
		HashMap<String, Word> result = new HashMap<String, Word>();
		for (Map.Entry<String, List<Pronunciation>> entry : pronunciationList
				.entrySet()) {
	...
	public Word getWord(String text) {
		...
		if (word == null) {
			...
			if (wordReplacement != null) {
				...
				logger.warn("Replacing " + text + " with " + wordReplacement);
				if (word == null) {
					logger.error("Replacement word " + wordReplacement
							+ " not found!");
				}
	...
	public String toString() {
		return super.toString() + "numWords=" + wordDictionary.size()
				+ " dictLlocation=" + getWordDictionaryFile();
	}
	...
	public Word[] getFillerWords() {
		return fillerDictionary.values().toArray(
				new Word[fillerDictionary.size()]);
	}
}
