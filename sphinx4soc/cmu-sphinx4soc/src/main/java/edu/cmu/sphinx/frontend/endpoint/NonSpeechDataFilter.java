...
public class NonSpeechDataFilter extends BaseDataProcessor
{
    ...
    public Data getData ( Data audioData ) throws DataProcessingException
    {
//        Data input = audioData.clone();
        System.out.println( "NonSpeechDataFilter" );
        if ( audioData instanceof SpeechStartSignal ) {
            inSpeech = true;
        }

        if ( audioData instanceof SpeechEndSignal ) {
            inSpeech = false;
        }

        while ( audioData!=null
                &&!( audioData instanceof DataEndSignal )&&!( audioData instanceof DataStartSignal )
                &&!( audioData instanceof SpeechEndSignal )&&!inSpeech ) {
            if ( audioData instanceof SpeechStartSignal ) {
                inSpeech = true;
            }
