...
import edu.cmu.sphinx.util.LoadResourceUtils;
...
public class FastDictionary implements Dictionary {
	/**
	 * The property for the custom dictionary file paths. This addenda property
	 * points to a possibly empty list of URLs to dictionary addenda. Each
	 * addendum should contain word pronunciations in the same Sphinx-3
	 * dictionary format as the main dictionary. Words in the addendum are added
	 * after the words in the main dictionary and will override previously
	 * specified pronunciations. If you wish to extend the set of pronunciations
	 * for a particular word, add a new pronunciation by number. For example, in
	 * the following addendum, given that the aforementioned main dictionary is
	 * specified, the pronunciation for 'EIGHT' will be overridden by the
	 * addenda, while the pronunciation for 'SIX' and 'ZERO' will be augmented
	 * and a new pronunciation for 'ELEVEN' will be added.
	 * 
	 * <pre>
	 *          EIGHT   OW T
	 *          SIX(2)  Z IH K S
	...
	// Configuration data
	// --------------------------------
	static final Logger logger = Logger.getLogger(FastDictionary.class);
	...
	public FastDictionary(String wordDictionaryFile,
		...
		this(LoadResourceUtils.load(wordDictionaryFile),
				LoadResourceUtils.load(fillerDictionaryFile),
	...
	public void newProperties(PropertySheet ps) throws PropertyException {
		...
		wordDictionaryFile = ConfigurationManagerUtils.getResource(
				PROP_DICTIONARY, ps);
		fillerDictionaryFile = ConfigurationManagerUtils.getResource(
				PROP_FILLER_DICTIONARY, ps);
	...
	public void allocate() throws IOException {
		logger.info("Allocating FastDictionary");
		if (!allocated) {
			...
			if (addendaUrlList != null || !addendaUrlList.isEmpty()) {
		...
		logger.info("FastDictionary Allocated ");
	...
	/**
	 * Creates a list of resource URLs given the name of the property. This
	 * method should eventually be replaced by a corresponding version in
	 * PropertySheet
	 * 
		...
		if (pathListString != null) {
			for (String addendumPath : pathListString.split(";")) {
				try {
					...
					throw new IllegalArgumentException("Addendum path: "
							+ addendumPath + " is not a valid URL.");
				}
			}
		}
		return resourceList;
